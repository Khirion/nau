...
void main() {
    ...
    if (control == 0)
        ...
        outColor = vec4(max(texture(blur,texCoordV).rgb * 1.5 + texture(mainBlur,texCoordV).rgb * 2, texture(branchBlur, texCoordV).rgb) + textureLod(blur, texCoordV, 8).rgb * 7 * vec3(0.0, 0.30, 0.60), 1.0) * 2;
    }
}