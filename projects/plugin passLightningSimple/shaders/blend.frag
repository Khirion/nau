...
void main() {
    ...
    switch (control){
        ...
        case 1:
            f *= 2.5;
            outColor = vec4(texture(blurUnit,texCoordV).rgb * (1-f) + texture(sceneUnit,texCoordV).rgb * (f), 1.0);
            break;
        
        case 2:
            f *= 2.5;
            outColor = vec4(texture(blurUnit,texCoordV).rgb * (f+0.5) + texture(sceneUnit,texCoordV).rgb * (1-(f+0.5)), 1.0);
            break;
